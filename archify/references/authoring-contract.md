...
English or `"zh-TW"` for Traditional Chinese. The renderer consumes the authored
...
For a requested language outside `en` and `zh-TW`, do not write an unsupported
...
`zh-TW` for another language or Chinese locale.

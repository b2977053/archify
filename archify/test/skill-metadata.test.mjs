...
test('language behavior stays within the bounded locale contract', () => {
  ...
  assert.match(skill, /use `"en"` or `"zh-TW"`/);
  ...
  assert.match(authoringContract, /outside `en` and `zh-TW`/);
  assert.match(authoringContract, /artifact is\s+not fully localized/);
  assert.match(authoringContract, /Do not silently substitute\s+`zh-TW` for another language or Chinese locale/);

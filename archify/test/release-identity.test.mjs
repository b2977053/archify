...
function writeValidDevelopmentFixture(root, overrides = {}) {
  ...
  const chinese = [
    '![開發版本](https://img.shields.io/badge/version-2.13.0--dev.0-blue)',
    '',
    `當前開發版本：\`v${version}\``,
    '',
    'Raven 使用 ZIP 手動安裝：將 archify.zip 解壓到 `~/.raven/workspace/skills`，解壓後會得到 `~/.raven/workspace/skills/archify`；Raven 不屬於 Agent 切換器目標。',
  ].join('\n');
  const files = {
    ...
    'scripts/start-template.html': 'development · 開發版 · [[ARCHIFY_VERSION]]',
    'scripts/guide-template.html': 'development · 開發版 · [[ARCHIFY_VERSION]]',
    'scripts/gallery-template.html': 'development · 開發版 · [[ARCHIFY_VERSION]]',
    'docs/index.html': `<span>development · v${version} · 開發版 · 9/9 checks</span><p>Raven manual ZIP / ZIP 手動安裝: extract archify.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/archify; 將 archify.zip 解壓到 ~/.raven/workspace/skills，解壓後會得到 ~/.raven/workspace/skills/archify; not an agent-switcher target.</p>`,
    'docs/start.html': `<span>development · v${version} · 開發版</span><p>Raven manual ZIP / ZIP 手動安裝: extract archify.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/archify; 將 archify.zip 解壓到 ~/.raven/workspace/skills，解壓後會得到 ~/.raven/workspace/skills/archify; not an agent-switcher target.</p>`,
...
function writeValidStableFixture(root, overrides = {}) {
  ...
  const chinese = [
    '![穩定版本](https://img.shields.io/badge/version-2.13.0-blue)',
    '',
    `當前穩定版本：\`v${version}\``,
    '',
    'Raven 使用 ZIP 手動安裝：將 archify.zip 解壓到 `~/.raven/workspace/skills`，解壓後會得到 `~/.raven/workspace/skills/archify`；Raven 不屬於 Agent 切換器目標。',
  ...
  const files = {
    ...
    'README_EN.md': english,
    'README_ZH.md': chinese,
    'scripts/start-template.html': 'stable · 穩定版 · [[ARCHIFY_VERSION]]',
    'scripts/guide-template.html': 'stable · 穩定版 · [[ARCHIFY_VERSION]]',
    'scripts/gallery-template.html': 'stable · 穩定版 · [[ARCHIFY_VERSION]]',
    'docs/index.html': `<span>stable · v${version} · 穩定版 · 9/9 checks</span><p>Raven manual ZIP / ZIP 手動安裝: extract archify.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/archify; 將 archify.zip 解壓到 ~/.raven/workspace/skills，解壓後會得到 ~/.raven/workspace/skills/archify; not an agent-switcher target.</p>`,
    'docs/start.html': `<span>stable · v${version} · 穩定版</span><p>Raven manual ZIP / ZIP 手動安裝: extract archify.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/archify; 將 archify.zip 解壓到 ~/.raven/workspace/skills，解壓後會得到 ~/.raven/workspace/skills/archify; not an agent-switcher target.</p>`,
...
test('landing proof receipt matches the current nine-check artifact contract', () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'docs/index.html': '<span>development · v2.13.0-dev.0 · 開發版 · 8/8 checks</span><p>Raven manual ZIP / ZIP 手動安裝: extract archify.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/archify; 將 archify.zip 解壓到 ~/.raven/workspace/skills，解壓後會得到 ~/.raven/workspace/skills/archify; not an agent-switcher target.</p>',
    });
...
test('landing rejects every stale N/N contract count even when 9/9 is also present', () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'docs/index.html': [
        '<span>development · v2.13.0-dev.0 · 開發版 · 9/9 checks</span>',
        '<span>legacy receipt · 7/7 checks</span>',
        '<p>Raven manual ZIP / ZIP 手動安裝: extract archify.zip into ~/.raven/workspace/skills, which yields ~/.raven/workspace/skills/archify; 將 archify.zip 解壓到 ~/.raven/workspace/skills，解壓後會得到 ~/.raven/workspace/skills/archify; not an agent-switcher target.</p>',
      ].join('\n'),
    });
...
test('Raven stays a truthful manual ZIP install and never becomes a generated agent-switcher command', () => {
  ...
  try {
    writeValidDevelopmentFixture(fixture, {
      'docs/start.html': [
        '<span>development · v2.13.0-dev.0 · 開發版</span>',
...
test('generated public-page templates keep a development marker and version placeholder', () => {
  ...
  try {
    ...
    const result = runCheck(fixture);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /scripts\/gallery-template\.html must use \[\[ARCHIFY_VERSION\]\] with development and 開發版 labels/);
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
  ...
  try {
    writeValidStableFixture(fixture, {
      'scripts/guide-template.html': [
        ...
        "versionLabel:'場景指南 / 穩定版 / v[[ARCHIFY_VERSION]]'",
      ].join('\n'),
    });

    const result = runCheck(fixture);
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /scripts\/guide-template\.html must not label \[\[ARCHIFY_VERSION\]\] as development or 開發版/);
  } finally {
    fs.rmSync(fixture, { recursive: true, force: true });
  }
});
